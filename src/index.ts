...
const HELP = `
Scraper de Jurisprudencia del Poder Judicial del Perú. Busca un término, recorre
todas las páginas de resultados, extrae los datos y descarga los PDFs (manejando
429 con backoff exponencial).

Uso:
  npm run scrape              busca, recorre páginas y descarga PDFs
...
Variables de entorno:
  SEARCH_TERM="despido"       término a buscar (default "contrato de trabajo")
...
Salida:
  output/data.json            resoluciones extraídas (acumulado, dedup por uuid)
...
Ejemplos:
  ...
  SEARCH_TERM="despido arbitrario" MAX_PAGES=5 DOWNLOAD_PDFS=0 npm run scrape
`;
